...
def motorcontrol():
        ...
        print accumleft, accumright
        ## Start Commented section
        if accumleft == 0 and accumright == 0:
                stop()
        if accumright < 0:
           right()
           for i in range(1, abs(accumright)):
               motor_bwd()
        if accumleft < 0:
           left()
           for i in range(1, abs(accumleft)):
               motor_bwd()
        if accumright > 0:
           right()
           for i in range(1, accumright):
               motor_fwd()
        if accumleft > 0:
           left()
           for i in range(1, accumleft):
              motor_fwd()
...
### Use CNTRL-C to stop the program
    while True:
        ...
        ## use something like "dist = 21" if you want to stop nose stimulation
        dist = us_dist(15)
        ## End Comment
        if dist<25:
            print "OBSTACLE (Nose Touch)"
